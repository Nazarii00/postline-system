...
const OperatorLayout = () => {
  ...
  const subtitle = user?.departmentId
    ? `ID відділення: ${user.departmentId}`
    : 'Робоча панель';
    ...
    <div className="h-screen bg-slate-100 flex font-sans text-slate-900 relative overflow-hidden">
      ...
      <aside className="hidden lg:flex w-72 bg-white/80 backdrop-blur border-r border-slate-200 flex-col z-20 transition-all h-full">
        ...
        <div className="p-6 border-t border-slate-100 shrink-0">
          <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 w-full text-red-500 hover:bg-red-50 rounded-2xl transition-all font-bold text-sm active:scale-95">
            <LogOut size={20} />
            Вийти
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed inset-y-0 left-0 w-72 bg-white/95 backdrop-blur-md shadow-2xl p-6 flex flex-col animate-in slide-in-from-left duration-300">
            ...
            <div className="mt-auto pt-6 border-t border-slate-100 shrink-0">
              <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 w-full text-red-500 hover:bg-red-50 rounded-xl transition-all font-bold text-sm active:scale-95">
                <LogOut size={20} />
                Вийти
              </button>
            </div>
