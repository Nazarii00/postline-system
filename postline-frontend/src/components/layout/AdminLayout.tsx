...
const AdminLayout = () => {
  ...
  const menuItems = [
    { icon: <LayoutDashboard size={20} />, label: "Огляд системи", path: "/admin" },
  ...
  return (
    // Жорстко фіксуємо висоту на весь екран, забороняємо загальний скрол
    <div className="h-screen bg-slate-100 flex font-sans text-slate-900 relative overflow-hidden">
      
      {/* Декоративний фон (уніфікований) */}
      ...
      {/* Sidebar для десктопа (на всю висоту батька) */}
      <aside className="hidden lg:flex w-72 bg-white/80 backdrop-blur border-r border-slate-200 flex-col z-20 transition-all h-full">
        <div className="p-8 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-pine rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-pine/20">P</div>
            <span className="text-xl font-bold text-slate-900 tracking-tight">PostLine</span>
          </div>
        </div>

        <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
          <p className="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Меню керування</p>
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all font-bold text-sm group ${
                  ...
                  : 'text-slate-500 hover:text-pine hover:bg-slate-50'
                }`}
              >
        ...
        <div className="p-6 border-t border-slate-100 shrink-0">
          <button className="flex items-center gap-3 px-4 py-3 w-full text-red-500 hover:bg-red-50 rounded-2xl transition-all font-bold text-sm active:scale-95">
            <LogOut size={20} />
            Вийти
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10 h-full">
        ...
        {/* Header (фіксована висота, не стискається) */}
        <header className="h-20 shrink-0 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 md:px-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest">
              Адмін-панель <span className="text-slate-300">/</span> <span className="text-pine">Керування</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 pl-2">
              <div className="text-right hidden sm:block">
                ...
                <p className="text-[10px] font-medium text-slate-400 mt-1 uppercase">Головний офіс</p>
              </div>
        ...
        {/* Контентна зона - ТІЛЬКИ ВОНА СКРОЛИТЬСЯ */}
        <main className="flex-1 relative p-4 md:p-8 overflow-y-auto custom-scrollbar">
          {/* Outlet замість children автоматично рендерить вкладені роути */}
          <Outlet />
        </main>
      </div>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed inset-y-0 left-0 w-72 bg-white/95 backdrop-blur-md shadow-2xl p-6 flex flex-col animate-in slide-in-from-left duration-300">
            <div className="flex justify-between items-center mb-10 shrink-0">
               <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-pine rounded-lg flex items-center justify-center text-white font-black shadow-md shadow-pine/20">P</div>
                  <span className="font-bold text-lg text-slate-900">PostLine</span>
               </div>
               <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-slate-400 hover:text-pine hover:bg-slate-100 rounded-lg transition-colors">
                 <X size={20} />
               </button>
            </div>
            ...
            <div className="mt-auto pt-6 border-t border-slate-100 shrink-0">
              <button className="flex items-center gap-3 px-4 py-3 w-full text-red-500 hover:bg-red-50 rounded-xl transition-all font-bold text-sm active:scale-95">
                <LogOut size={20} />
                Вийти
              </button>
            </div>
